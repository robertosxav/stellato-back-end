...
public interface MaterialCotadoRepository extends JpaRepository<MaterialCotadoModel, Long>{
	...
	@Query("select new com.stellato.vendas.domain.materialCotado.entity.MaterialCotadoEntity(m.id,m.codigo,m.observacao,m.valor,m.idFornecedor,"
			+ "m.razaoSocial,m.cnpj,m.idMaterial,m.descricaoMaterial,m.status,m.criadoEm,m.criadoPor,m.alteradoEm,m.alteradoPor) from MaterialCotadoModel m")
	public List<MaterialCotadoEntity> listarTodos();
	
	@Query("select new com.stellato.vendas.domain.materialCotado.entity.MaterialCotadoEntity(m.id,m.codigo,m.observacao,m.valor,m.idFornecedor,"
			+ "m.razaoSocial,m.cnpj,m.idMaterial,m.descricaoMaterial,m.status,m.criadoEm,m.criadoPor,m.alteradoEm,m.alteradoPor) from MaterialCotadoModel m where m.status = 1")
